...
private:
	...
	static bool queue_has_place(const std::vector<size_t> &batch_queue_added_size, const size_t &filesize,
	                            const size_t &max_filesize, const uint32_t &current_batch_add_start);

public:
	static void save_config(const bool config_scan_dot_paths, const std::filesystem::path &config_path_to_scan,
...
public:
	...
	static int add(std::vector<std::string> &paths, const size_t &paths_size_l, std::vector<uint32_t> &paths_count,
	               const size_t &path_count_size_l, std::vector<words_reversed> &words_reversed_l,
