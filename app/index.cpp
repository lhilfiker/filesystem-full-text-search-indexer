...
void index::save_config(const std::filesystem::path &config_index_path, const int config_buffer_size) {
	...
	if (buffer_size < 10000) {
		buffer_size = 10000;
		log::write(3, "index: save_config: buffer size needs to be at least 10000. setting it to 10000.");
	}
...
void index::resize(const std::filesystem::path &path_to_resize, const int size) {
	...
	if (ec) {
		log::error(
			"index: resize: could not resize file at path: " + (path_to_resize).string() + " with size: " +
			std::to_string(size) + ".");
	}
...
int index::get_actual_size(const mio::mmap_sink &mmap) {
	...
	if (ec) {
		log::write(3, "index: get_actual_size: error when counting actual size");
		return -1;
	}
...
void index::check_files() {
	...
	if (!std::filesystem::exists(index_path / "paths.index") || helper::file_size(index_path / "paths.index") == 0 || !
	    std::filesystem::exists(index_path / "paths_count.index") || helper::file_size(index_path / "paths_count.index")
	    == 0 || !std::filesystem::exists(index_path / "words.index") || helper::file_size(index_path / "words.index") ==
	    0 || !std::filesystem::exists(index_path / "words_f.index") || helper::file_size(index_path / "words_f.index")
	    == 0 || !std::filesystem::exists(index_path / "reversed.index") ||
	    helper::file_size(index_path / "reversed.index") == 0 || !
	    std::filesystem::exists(index_path / "additional.index") || helper::file_size(index_path / "additional.index")
	    == 0) {
		first_time = true;
...
int index::initialize() {
	...
	is_mapped = false;
	std::error_code ec;
	unmap(); //unmap anyway in case they are already mapped.
	...
	if (int paths_size = get_actual_size(mmap_paths);
		paths_size == -1 && helper::file_size(index_path / "paths.index") > 0) paths_size = 0;
	if (int paths_count_size = get_actual_size(mmap_paths);
		paths_count_size == -1 && helper::file_size(index_path / "paths_count.index") > 0) paths_count_size = 0;
	if (int words_size = get_actual_size(mmap_words);
		words_size == -1 && helper::file_size(index_path / "words.index") > 0) words_size = 0;
	if (int words_f_size = get_actual_size(mmap_words_f);
		words_f_size == -1 && helper::file_size(index_path / "words_f.index") > 0) words_f_size = 0;
	if (int reversed_size = get_actual_size(mmap_reversed);
		reversed_size == -1 && helper::file_size(index_path / "reversed.index") > 0) reversed_size = 0;
	if (int additional_size = get_actual_size(mmap_additional);
		additional_size == -1 && helper::file_size(index_path / "additional.index") > 0) additional_size = 0;
...
int index::add(std::vector<std::string> &paths, const size_t &paths_size_l, std::vector<uint32_t> &paths_count,
               const size_t &paths_count_size_l, std::vector<words_reversed> &words_reversed_l,
	...
	if (first_time) {
		...
		words_size_buffer = (((words_size_l + words_reversed_l.size()) * 5) / 8) + 5;
		// we save one char  as 5 bits instead of 8 (1 byte) + place for end of file char and buffer.
		// words_f_size maybe needs to be extended to allow larger numbers if 8 bytes turn out to be too small. maybe automatically resize if running out of space?
		...
		for (const words_reversed &additional_reversed_counter: words_reversed_l) {
			if (additional_reversed_counter.reversed.size() < 5) continue;
			// if under 4 words, no additional is required.
		...
		for (const words_reversed &reversed: words_reversed_l) {
			...
			if (reversed.reversed.size() <= 4) {
				for (int i = 0; i < reversed.reversed.size(); ++i) {
					current_reversed_block.ids[i] = reversed.reversed[i] + 1;
					//paths are indexed from 1 because 0 is reserved for empty values.
				}
			} else {
				int additional_i = 0;
				int reversed_i = 0;
			...
			for (char byte: current_reversed_block.bytes) {
				mmap_reversed[file_location] = byte;
				++file_location;
			}
		}
		reversed_size = file_location;
		log::write(2, "indexer: add: reversed and additional written");
